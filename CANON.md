...
implements one exact source-derived candidate over complete finite ordered
traces: p=(2i+1)/(2n), u=2p-1, and lifted turns t=2p. That candidate is not
...
cover for all fourteen minimum-packet initiations. The merged v0.8 experiment
placed transverse witnesses `-1`, `0`, and `+1` beside that unchanged root
chart under reversible local-frame and global-side descriptions. v0.9 corrects
...
odd-motion failure witnesses. v0.11 preserves the surviving candidate as exact
rational transverse, breadth, and lifted-turn evidence with an exact inverse.
It also retains explicit breadth and lifted-turn witnesses proving that
binary64 `LiftedCarrierPoint` materialization is not injective over the
...
finite ordered source address. For occurrence i in complete scope cardinality
n, the law fixes p=(2i+1)/(2n), u=2p-1, and t=2p. It retains exact v0.17 trace
...
19. a corrective exact-rational transverse envelope over the v0.7 root chart,
...
20. a bounded carrier-coordinate admissibility experiment over four declared
    ...
    and
21. an exact-coordinate representation boundary that preserves the signed-local
    affine law and inverse over the declared rational transverse interval,
    retains fixed provenance, classifies binary64 points as lossy renderings,
...
28. an ordered-source coordinate derivation boundary that maps occurrence i in
    one complete finite ordered scope n to exact p=(2i+1)/(2n), u=2p-1, and
...
for EDCM. Its bounded framed root loop has a reversible exact chart into the
...
specific failures. v0.11 then gives the signed-local law an exact rational
...
finite-ordered-source midpoint derivation candidate while leaving its selection
and cross-scope composition explicit. The total Structural Null relationship,
