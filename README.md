...
hmmm: the exact EDCM observation floor is executable and the completion-motion
root is now preserved. The next truth obligations are reconstruction of the
circle/epicycle/disk/sphere motion law from the pinned A0-betatest specimen,
formal Möbius integration, higher-gonol composition, deterministic full-corpus
runs, and failure analysis that does not allow the present metric projections to
select the geometry that validates them.
