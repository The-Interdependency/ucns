...
`ucns_v06_fix.py`, on objects of finite nesting depth.
...
**Empirical verification (proof_trace.py).** The trace records actual call
depths on two cases:
...
- Empirical (depth 1): 900/900 Class III cases recovered by `left_quotient`
  with `catalogue=None` (class3_rerun_v06.py).
- Empirical (depth 2): the d(A2)=2 oracle case in proof_trace.py recovers
  cleanly with max recursion depth exactly `1 + d(A2) = 3`, matching
  Lemma 7's bound.
- Structural: this document.
- Tests: test_ucns_v06_fix.py, all 9 tests green; cancellativity 0
  violations across 5,508 pairs (flat) + 5,508 pairs (depth-1).
