...
| `IMPL-10` | Within one authority-bound complete finite ordered trace, occurrence i of cardinality n derives exact p=(2i+1)/(2n), u=2p-1, and t=2p; initiated outcomes receive the exact circle candidate and other outcomes retain explicit blockers. | implemented-candidate | v0.19 trace-local source-coordinate derivation | `src/ucns/source_coordinate.py`; `SOURCE_COORDINATE_DERIVATION_V019.md`; exact within-scope injectivity, a producer-issued receipt verified against the exact v0.17 authority report, and upstream object identity, with no caller-mintable authority fields, selection, cross-scope composition, higher geometry, completion, or activation |
