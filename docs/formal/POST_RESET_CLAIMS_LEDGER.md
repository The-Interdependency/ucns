...
| `IMPL-10` | Within the fixed full producer-owned finite ordered v0.17 demonstration trace, occurrence i of cardinality n derives exact p=(2i+1)/(2n), u=2p-1, and t=2p; initiated outcomes receive the exact circle candidate and other outcomes retain explicit blockers. | implemented-candidate | v0.19 trace-local source-coordinate derivation | `src/ucns/source_coordinate.py`; `SOURCE_COORDINATE_DERIVATION_V019.md`; exact within-scope injectivity, a producer-issued receipt verified against the exact full v0.16 admission trace and every ordered v0.17 disposition, evidence tuple, initiation receipt, rejection, and trace identity, with no caller-mintable authority fields, consistent multi-layer prefixes, id-preserving outcome changes, selection, cross-scope composition, higher geometry, completion, or activation |
...
| `NEG-8` | Content, digest, runtime identity, A0 lanes, carrier position alone, scalar projection, binary64 rendering, caller-minted authority fields, and an unbound tuple length are not inputs to the v0.19 source-coordinate law. | negative-result | v0.19 finite ordered trace law | exact code path derives only from occurrence index and producer-receipted complete scope cardinality |
