...
> **Status:** v0.19 authority, ordered-source coordinate candidate derivation, explicit-input candidate application, assignment-admission evidence, gonol-initiation evidence, analytic non-null certificates, partial root attachment, and full-corpus execution ledger. It records standing; it does not promote any candidate.
>
...
| `A14` | The fixed full producer-owned finite ordered v0.17 demonstration trace supplies one exact source-coordinate candidate: occurrence i in cardinality n derives p=(2i+1)/(2n), u=2p-1, and t=2p; initiated outcomes receive the exact circle candidate and other outcomes remain explicit blockers. | implemented-evidence | `SOURCE_COORDINATE_DERIVATION_V019.md` and `src/ucns/source_coordinate.py`. The law is exact and injective within the producer-receipted complete scope, whose receipt binds the exact full v0.16 admission trace and every ordered v0.17 disposition, evidence tuple, initiation receipt, rejection, and trace identity. It preserves equal-content multiplicity and exact upstream objects and rejects caller-supplied authority fields, consistent multi-layer prefixes, id-preserving outcome changes, content-derived geometry, and unbound tuple length. Additional scopes require versioned producer declarations. It is not selected and supplies no cross-scope composition, total topology, higher geometry, completion, or activation. |
...
| `M18` | The v0.19 ordered-source midpoint law can apply the v0.11 exact signed-local circle candidate reversibly to every explicitly initiated outcome in the fixed full producer-owned v0.17 demonstration trace. | implemented-evidence | The source-coordinate receipt retains the exact full v0.16 admission trace, every complete ordered v0.17 outcome evidence identity, exact source address, Fraction-valued coordinate and inverse, native frame, sign-derived side, candidate law standing, and linked lossy rendering. The v0.18 independent proposal remains separate nonselected evidence; additional scopes require new versioned producer declarations. |
...
initiation boundary, v0.18 explicit-input application, and v0.19 ordered-source
coordinate candidate:

1. selection and external grounding of a source-coordinate law, cross-scope
   stability and higher-gonol composition, and the total Structural
   Null-to-carrier relationship beyond the finite trace-local midpoint
   candidate, explicit-input application, admitted outcomes, and marked root;
...
runtime scope. v0.16 closes the prior admission question: arbitrary-domain
occurrences enter through explicit adapters and each receives one exact tagged
outcome without identity-derived geometry. v0.17 now makes the next bounded
causal doorway executable: one admitted word moves from typed Structural Null
...
and rendering-loss evidence. v0.19 adds the exact finite ordered-source
midpoint derivation candidate and explicit blockers. Candidate selection,
cross-scope and higher-gonol composition, total Structural Null topology,
canonical breadth, scoped completion, and evidence that the exact candidate
supplies the complete carrier remain absent.
