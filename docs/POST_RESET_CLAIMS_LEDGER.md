...
| `M18` | The v0.19 ordered-source midpoint law can apply the v0.11 exact signed-local circle candidate reversibly to every explicitly initiated outcome in one authority-bound complete finite trace. | implemented-evidence | Source-coordinate receipts retain the producer-issued scope-exhaustion receipt and exact v0.17 authority report, exact source address, Fraction-valued coordinate and inverse, native frame, sign-derived side, candidate law standing, and linked lossy rendering. The v0.18 independent proposal remains separate nonselected evidence. |
