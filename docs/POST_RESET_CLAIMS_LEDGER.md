...
| `A14` | An authority-bound complete finite ordered v0.17 trace supplies one exact source-coordinate candidate: occurrence i in cardinality n derives p=(2i+1)/(2n), u=2p-1, and t=2p; initiated outcomes receive the exact circle candidate and other outcomes remain explicit blockers. | implemented-evidence | `SOURCE_COORDINATE_DERIVATION_V019.md` and `src/ucns/source_coordinate.py`. The law is exact and injective within one producer-receipted complete scope, preserves the exact v0.17 authority report, equal-content multiplicity, and exact upstream objects, and uses no caller-supplied authority fields, content identity, digest, or unbound tuple length. It is not selected and supplies no cross-scope composition, total topology, higher geometry, completion, or activation. |
