...
| `factor_search_v08(P, catalogue=None)` | `IMPLEMENTED` + `TEST-BACKED` exhaustive | `ucns.factor_search_v08`, re-exported from `ucns` | v1.0 factorization engine: witness-matrix recursive quotient solver. Since the v1.0 completion handoff it enumerates **every** catalogue-bounded payload assignment and face option (`payload_system.iter_payload_system_solutions`, brute-force-equal on bounded oracles); `SEQ-PRIME` is reached only through demonstrable exhaustion — never truncation, never a caught exception. Executable catalogue-completeness is evidence by exhaustion of the finite supplied catalogue; it is not a Lean proof. |
| Negative-result certification | `IMPLEMENTED` + `TEST-BACKED` | `ucns.factorization_result`, `ucns.catalogue_certificate`, `ucns.factor_search_report` | `negative_result_certified` (alias `seq_prime_is_absolute`) requires: declared complete/oracle domain, non-unit target, demonstrated exhaustion, machine-checked catalogue coverage bound to the exact catalogue fingerprint and rule version, and coverage-preserving pruning only. Never set from a domain label alone; no caller assertion exists. Custom catalogues are uncertified unless structurally verified as canonical-exact/superset. |
| Oracle-atom classification | `IMPLEMENTED` + `TEST-BACKED` | `ucns.domains.is_oracle_atom` | Extensionally identical to structural membership in the canonical generated catalogue, by construction. Deliberately narrower than the geometric bounds (`in_domain`); the bounded-but-non-catalogue witness `[(0), (3/2)]` is non-oracle. Rule identity: `ORACLE_CATALOGUE_RULE_VERSION`. |
| Complete quotient surfaces | `IMPLEMENTED` + `TEST-BACKED` | `ucns.left_quotients` / `right_quotients` / `SolutionLimitExceeded` (public); singular `left_quotient` / `right_quotient` | Plural APIs expose the full finite solution sets; singular functions are deterministic compatibility selectors over them (first non-unit solution, `None` otherwise). `UCNSStore` retrieval and `enumerate_factorizations` consume complete solution sets; no greedy single-quotient path remains on any completeness-claiming surface. |
| Immutable nonempty object model | `IMPLEMENTED` + `TEST-BACKED` | `ucns.canonical.UCNSObject` | Recursively immutable canonical value: tuple storage, frozen attributes, constructor-normalized, hash consistent with equality; empty objects cannot be constructed — the runtime carrier is the nonempty normalized object set. |
| Typed dictionary keys | `IMPLEMENTED` + `TEST-BACKED` | `ucns.recursive_codec` | Versioned key capsules preserve exact key types (bytes, str, int, bool, finite float, tuples thereof); duplicate decoded keys raise instead of overwriting; legacy dictionaries still decode with bytes coercion but reject unhashable/colliding keys. Values keep the documented bytes coercion. |
