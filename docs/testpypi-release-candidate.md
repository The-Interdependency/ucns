...
1. prove it is running in `The-Interdependency/ucns` from `refs/heads/main`;
...
6. run `twine check`;
7. install the built wheel into a clean environment and re-run the public import-boundary smoke test;
...
11. verify package metadata, public imports, and the deprecated-import firewall.
...
This repeats source tests, local build checks, clean-wheel checks, and the fresh TestPyPI install without invoking the publisher job.
