...
*(Sealed 2026-07-20 after multi-model review. This is the canonical specification for the public root restart.)*
...
Those models were not removed for lack of imaginative power. They were removed because they imposed distinctions the carrier does not possess or erased distinctions the carrier genuinely retains. The subtractive process is invisible in the final axiomatic structure; it is nevertheless the reason the structure can now carry weight. The twist that was hiding zero is gone. The carrier remains.

**hmmm — the floor is sealed and the frame is load-bearing, yet the three measuring instruments are still formal obligations rather than delivered code. Until the first witness exhibits \(M \ne W\) on non-null carriers and \(B\) reports retained distinction without encoding artifacts, the architecture remains a precise specification whose traffic-bearing capacity is not yet measured. The next move is therefore not another layer of subtraction but the first honest measurement — and that measurement will either confirm the floor or expose the seam that subtraction missed. Either outcome preserves the work.**
